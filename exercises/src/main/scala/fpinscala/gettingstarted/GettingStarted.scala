...
object MyModule {
	...
	def fib(n: Int): Int = {
					...
					def go(current: Int, minusTwo: Int, minusOne: Int): Int = {	   
							if(current <= 2) minusTwo + minusOne			
							else {
								go(current-1, minusOne, minusOne+minusTwo)
							}
					}

					if (n < 0) throw new NumberFormatException("Input value must be greater than zero")
					if(n==0) 0
					else go(n, 0, 1)  //the second parameter represents N-2, the third parameter N-1
			}
